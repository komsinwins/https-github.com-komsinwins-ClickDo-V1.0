...
import { AlertTriangle, Calendar, MapPin, Plus, Briefcase, CheckCircle2, AlertCircle, FileText, TrendingUp, Sparkles, FolderOpen, LayoutGrid, List, TableProperties, Search, Filter, Database, Settings, BarChart3, PieChart, X, RotateCcw } from 'lucide-react';
...
export default function DashboardOverview({
  ...
  // Calculate project count per status
  ...
  const filteredProjects = React.useMemo(() => {
    return projects.filter((p) => {
      ...
      const matchQuery =
        ...
        (p.partnerCompany || '').toLowerCase().includes(q) ||
        (p.contractor?.teamName || '').toLowerCase().includes(q) ||
        (p.contractor?.foremanName || '').toLowerCase().includes(q) ||
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <h2 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
            ...
            <span className="text-white font-extrabold text-lg">รายการโครงการทั้งหมด ({filteredProjects.length})</span>
          </h2>
          <button
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-4 rounded-2xl shadow-xl space-y-3.5 no-print">
          ...
          <div className="flex flex-wrap items-center gap-3 flex-1">
            ...
            {/* Divider */}
            <span className="hidden sm:inline-block w-[1px] h-6 bg-zinc-800" />
          ...
          {/* Top Row: Search Input Field & Match Counter */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pb-3 border-b border-zinc-800">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="🔍 ค้นหาชื่อโครงการ, สถานที่ติดตั้ง, ชื่อ PM, ลูกค้า/ผู้ว่าจ้าง, เซลส์ หรือรหัสโครงการ..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-10 pr-9 py-2 text-xs font-semibold rounded-xl border border-zinc-700 bg-zinc-950 text-white placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-lime-500/50 focus:border-lime-500 transition-all font-sans"
              />
              {searchQuery && (
                <button
                  type="button"
                  ...
                  title="ล้างคำค้นหา"
                >
                  <X className="w-4 h-4 bg-zinc-800 hover:bg-zinc-700 rounded-full p-0.5" />
                </button>
              )}
            </div>

            {/* Reset Filters / Counter Button */}
            <div className="flex items-center gap-2 shrink-0">
              <span className="text-[11px] font-medium text-zinc-400 bg-zinc-950 px-3 py-1.5 rounded-xl border border-zinc-800">
                แสดง <b className="text-lime-400 font-mono font-bold">{filteredProjects.length}</b> จาก <b className="text-white font-mono">{projects.length}</b> โครงการ
              </span>
              {(searchQuery !== '' || statusFilter !== 'All') && (
                <button
                  ...
                  className="px-3 py-1.5 bg-zinc-800 hover:bg-zinc-750 text-zinc-300 hover:text-white rounded-xl text-xs font-bold border border-zinc-700 flex items-center gap-1.5 transition-all cursor-pointer"
