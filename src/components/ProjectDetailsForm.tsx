...
import { Save, Calendar, MapPin, User, Building2, Tag, ShieldCheck, Clock, X, Plus, Trash2, AlertTriangle, FileText, Users } from 'lucide-react';
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Customer Database Reference Dropdown */}
        {customers && customers.length > 0 && (
          <div className="bg-zinc-900/40 border border-zinc-800 rounded-xl p-3.5 space-y-2">
            <label className="text-[10px] font-bold text-lime-400 uppercase tracking-wider flex items-center gap-1.5">
              <Users className="w-3.5 h-3.5" />
              <span>ดึงข้อมูลด่วนจากฐานข้อมูลลูกค้า (Customer Database Link)</span>
            </label>
            <select
              onChange={(e) => {
                const selectedCust = customers.find(c => c.companyName === e.target.value);
                if (selectedCust) {
                  setOwnerName(selectedCust.companyName);
                  setPartnerCompany(selectedCust.contactPerson);
                }
              }}
              value={customers.some(c => c.companyName === ownerName) ? ownerName : ''}
              className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-lime-500 transition-all cursor-pointer"
            >
              <option value="">-- เลือกบริษัทลูกค้า เพื่อกรอกข้อมูลอัตโนมัติ --</option>
              {customers.map(c => (
            ...
            <p className="text-[10px] text-zinc-500">
              * ระบบจะเชื่อมโยงโครงการนี้กับข้อมูลบริษัทลูกค้าในฐานข้อมูลหลักให้อัตโนมัติ
            </p>
          </div>
        )}
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              required
              list="customers-list"
              placeholder="ระบุชื่อบริษัทลูกค้าหลัก หรือเลือกจากฐานข้อมูล"
              value={ownerName}
              onChange={(e) => {
                const val = e.target.value;
                setOwnerName(val);
                // Try to find matching customer and auto-populate partnerCompany
                const matchingCustomer = customers.find(
                  (c) => c.companyName.toLowerCase() === val.trim().toLowerCase()
                );
                if (matchingCustomer) {
                  setPartnerCompany(matchingCustomer.contactPerson);
                }
              }}
              className="w-full bg-zinc-950 border border-zinc-800 rounded-lg pl-10 pr-4 py-2.5 text-white placeholder-zinc-500 focus:outline-none focus:border-emerald-500 transition-all"
            />
            <datalist id="customers-list">
              {customers.map((cust) => (
                <option key={cust.id} value={cust.companyName}>
                  {cust.contactPerson ? `${cust.companyName} (${cust.contactPerson})` : cust.companyName}
                </option>
              ))}
            </datalist>
          </div>
